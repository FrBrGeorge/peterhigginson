...
var lineNo;
var step1Code = 0;
halted = "Program HALTED. STOP, LOAD or EDIT";
...
function textToHtml() {
    ...
    for (var n = 0; n < pLength; ++n) {
        ...
        if (letter == " " && (n + 1) < pLength && programText[n + 1] == " ") {
            continue
        }
        if (count == 0) {
            if (letter == "\n") {
            ...
            if (letter == "/" || letter == ";") {
                count = 11;
                comment = true;
                lineHTML += '<span class="comment">' + letter;
                lineEdit += letter;
                continue
            }
            var label = "" + letter;
            ...
            if (programText[n + i] != ":") {
                ...
                ++i;
                while ((n + i) < pLength) {
                    var c = programText[n + i];
                    if (c == "\n" || c == "/" || c == ";") {
                        justLabel = true;
                        break
                    }
                    if (c == "." && (n + i) < (pLength - 5)) {
                        if (programText.substring(n + i, n + i + 5).toUpperCase() == ".DATA") {
                            justLabel = true;
                            break
                        }
                    }
                        ...
                        if (pos2 == -1) {
                            ...
                            if (reg3 < 16) {
                                ...
                                if (reg3 == 20 && offsetText[r].substring(pos2 + 1, pos2 + 2) == "#") {
                                    ...
                                    if (a > 31 && (decode & 8) == 0) {
                                        error = "immediate must be 0-31 at line " + convert(r, instText[r]);
                                        break
                                    }
                                    if (a < 0) {
...
var instructions = ["ADD", "SUB", "CMP", "MOV", "AND", "ORR", "EOR", "LSR", "LSL", "B", "BEQ", "BNE", "BLT", "BGT", "LDR", "STR", "HALT", "MVN", "RFE", ".BLOCK", ".WORD", "BL", "PUSH", "POP", "RET", "LDRB", "STRB", ".BYTE", ".ASCII", ".ASCIZ", "ADDS", "SUBS", "BCS", "BVS", "BMI", ".ALIGN", "SVC", "BIS", "XOR", "OR", "JMS", "HLT"];
var instKey = [8 + 256, 8 + 256, 6, 6, 8 + 256, 8 + 256, 8 + 256, 256 + 2048, 256 + 2048, 128, 128, 128, 128, 128, 16 + 512, 16 + 512, 1, 6, 1, 1024, 1 + 128 + 1024, 128, 64, 64, 1, 16 + 512, 16 + 512, 1024, 4096, 4096, 8 + 256, 8 + 256, 128, 128, 128, 1024, 1025];
var inst1 = [3766484992, 3762290688, 3780116480, 3785359360, 3758096384, 3783262208, 3759144960, 3785359392, 3785359360, 3925868544, 167772160, 436207616, 3120562176, 3388997632, 3844014080, 3842965504, 3774873712, 3789553664, 4173138432, 0, 0, 3942645760, 3912040448, 3904700416, 3785420814, 3848208384, 3847159808, 0, 0, 0, 3767533568, 3763339264, 704643072, 1778384896, 1241513984, 0, 4009754624];
...
var startFm = [0, 6, 0, 10, 37, 16, 37, 20, 24, 26, 27, 35, 37, 37, 37, 36];
var newIValue = [3766484992, 3762290688, 3758096384, 3759144960, 3767533568, 3763339264, 3785359360, 3785359392, 3785359360, 3774873712, 3783262208, 3785359360, 3789553664, 3780116480, 3810525184, 3808428032, 3843031040, 3841982464, 3847225344, 3846176768, 3876585472, 3875536896, 3880779776, 3879731200, 3904700416, 4173138432, 3912040448, 3925868544, 167772160, 436207616, 3120562176, 3388997632, 704643072, 1778384896, 1241513984, 3942645760, 4009754624, 201326592, 0];
var newIMask = [4260364288, 4260364288, 4260364288, 4260364288, 4260364288, 4260364288, 4294905840, 4294901856, 4294901856, 4294967295, 4260364288, 4261347328, 4261347328, 4260425728, 4293918720, 4293918720, 4285530112, 4285530112, 4285530112, 4285530112, 4285530112, 4285530112, 4285530112, 4285530112, 4294901760, 4294967295, 4294901760, 4278190080, 4278190080, 4278190080, 4278190080, 4278190080, 4278190080, 4278190080, 4278190080, 4278190080, 4278190080, 201326592, 0];
var newINmame = ["ADD", "SUB", "AND", "EOR", "ADDS", "SUBS", "MOV", "LSR", "LSL", "HALT", "ORR", "MOV", "MVN", "CMP", "MOV", "MOV", "LDR", "STR", "LDRB", "STRB", "LDR", "STR", "LDRB", "STRB", "POP", "RFE", "PUSH", "B", "BEQ", "BNE", "BLT", "BGT", "BCS", "BVS", "BMI", "BL", "SVC", "MOV", "ILLEGAL"];
var newIDecode = [7, 7, 7, 7, 7, 7, 3, 5, 5, 0, 7, 3, 3, 4, 8, 8, 2, 2, 2, 2, 1, 1, 1, 1, 9, 0, 9, 6, 6, 6, 6, 6, 6, 6, 6, 6, 0, 10, 0];
